...
public class KafkaCreateTopicServiceTest {
	...
	GeneralConfigImpl config1, config2;
	...
	public void setUp() throws Exception {
		...
		config1 = new GeneralConfigImpl();
		config2 = new GeneralConfigImpl();
		topicDescr1 = new NewTopic("foo", 12, (short)1);
		topicDescr2 = new NewTopic("bar", 10, (short)3);
		service = new KafkaCreateTopicService(utilsMock1, config1, topicDescr1, 5000L);
	}
	
	@Test
	public void testGetters() {
		assertSame(utilsMock1, service.getUtils());
		assertSame(config1, service.getConfig());
	...
	public void testStart_ShouldCreateTopic() throws Exception {
		expect(utilsMock1.createAdmin(config1)).andReturn(adminMock);
	...
	public void testEquals() {
		...
		assertTrue(service.equals(new KafkaCreateTopicService(utilsMock1, config1, topicDescr1, 5000L)));
		assertFalse(service.equals(null));
		assertFalse(service.equals(this));
		assertFalse(service.equals(new KafkaCreateTopicService(utilsMock2, config1, topicDescr1, 5000L)));
		assertFalse(service.equals(new KafkaCreateTopicService(utilsMock1, config2, topicDescr1, 5000L)));
		assertFalse(service.equals(new KafkaCreateTopicService(utilsMock1, config1, topicDescr2, 5000L)));
		assertFalse(service.equals(new KafkaCreateTopicService(utilsMock1, config1, topicDescr1, 7000L)));
		assertFalse(service.equals(new KafkaCreateTopicService(utilsMock2, config2, topicDescr2, 7000L)));
	}
