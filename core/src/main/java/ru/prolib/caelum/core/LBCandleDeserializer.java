...
public class LBCandleDeserializer implements Deserializer<ILBCandle> {
	private final ByteUtils utils;
	
	public LBCandleDeserializer(ByteUtils utils) {
		this.utils = utils;
	}
	
	public LBCandleDeserializer() {
		this(ByteUtils.getInstance());
	}

	@Override
	public ILBCandle deserialize(String topic, byte[] bytes) {
		byte header = bytes[0];
		...
		case 2:
		{
			...
			return new LBCandle(open, high, low, close, (byte)(bytes[1] & 0b00001111),
