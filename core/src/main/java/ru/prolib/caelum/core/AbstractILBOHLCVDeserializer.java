...
public abstract class AbstractILBOHLCVDeserializer<T extends ILBOHLCV> implements Deserializer<T>  {
	protected final ByteUtils utils;
	
	public AbstractILBOHLCVDeserializer(ByteUtils utils) {
		this.utils = utils;
	}
	
	public AbstractILBOHLCVDeserializer() {
	...
	public T deserialize(String topic, byte[] bytes) {
		...
		case 2:
		{
			...
			OHLCVRecordType type = OHLCVRecordType.LONG_REGULAR;
			...
			if ( actual_volume_num_bytes == volume_num_bytes ) {
				...
				type = OHLCVRecordType.LONG_WIDEVOL;
