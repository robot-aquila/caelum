...
public class ItemDeserializer implements Deserializer<Item> {
	...
	public ItemDeserializer(ByteUtils utils) {
		this.utils = utils;
	}
	...
	@Override
	public Item deserialize(String topic, byte[] bytes) {
		byte header = bytes[0];
		switch ( 0b00000011 & header ) {
		case 0:
			throw new IllegalArgumentException("Record type not supported: 0");
		case 1:
			return new Item(utils.bytesToLong(bytes, 2, 2), (byte)(bytes[1] & 0x0F),
		...
		case 2:
			...
			return new Item(utils.bytesToLong(bytes, 2, value_num_bytes), (byte)(bytes[1] & 0x0F),
