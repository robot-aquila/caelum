...
public class KafkaTupleSerializer implements Serializer<KafkaTuple> {
	...
	public byte[] serialize(String topic, KafkaTuple tuple) {
		...
		// 1) pack open "as is"
		int abs_num_bytes = ByteUtils.longToByteArray(open, abs_bytes), rel_num_bytes;
		...
		abs_num_bytes = ByteUtils.longToByteArray(high, abs_bytes);
		rel_num_bytes = ByteUtils.longToByteArray(relative, rel_bytes);
		...
		relative = open - low;
		abs_num_bytes = ByteUtils.longToByteArray(low, abs_bytes);
		rel_num_bytes = ByteUtils.longToByteArray(relative, rel_bytes);
		if ( rel_num_bytes < abs_num_bytes ) {
			System.arraycopy(rel_bytes, 8 - rel_num_bytes, buffer, buffer_used_length, rel_num_bytes);
		...
		relative = open - close;
		abs_num_bytes = ByteUtils.longToByteArray(close, abs_bytes);
		rel_num_bytes = ByteUtils.longToByteArray(relative, rel_bytes);
		if ( rel_num_bytes < abs_num_bytes ) {
			System.arraycopy(rel_bytes, 8 - rel_num_bytes, buffer, buffer_used_length, rel_num_bytes);
		...
		if ( big_volume != null ) {
			...
			abs_num_bytes = ByteUtils.longToByteArray(volume, abs_bytes);
			System.arraycopy(abs_bytes, 8 - abs_num_bytes, buffer, buffer_used_length, abs_num_bytes);
